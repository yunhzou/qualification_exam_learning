...
\end{equation}
With the weighting used here, the implementation first averages this quantity over the
trainable tokens in each response, then averages equally over responses, and minimizes the
negative result. The clipping coefficient $\epsilon$ controls short-term reuse of one
behavior batch; the KL coefficient $\beta$ controls long-term movement from the reference
model. These averages recover the complete loss in Equation~\ref{eq:grpo-full-loss}.
