...
\subsection{Context compaction creates multiple training traces}
...
The required split also changes the shape of the training batch. One compacted coding
rollout may now produce several valid traces, while another produces fewer traces of very
different lengths. GLM-5.2 treats this ragged multi-trace structure as the reason to move
from same-prompt group comparison to critic-based PPO: each rollout can train
independently, the critic estimates token-level advantages, and a token-level loss handles
length imbalance \citep{glm52team2026}. Thus compaction creates two connected problems.
Exact per-call contexts are required to keep every trace on-policy; after those traces are
constructed correctly, the critic supplies localized credit without requiring equal-sized
GRPO sibling groups.
