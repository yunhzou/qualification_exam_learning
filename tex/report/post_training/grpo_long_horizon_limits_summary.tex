...
\subsection{Why GLM-5.2 moves to individual-rollout PPO}

GLM-5.2 cites a different problem. Compaction splits each long coding rollout into a
variable number of unequal sub-traces, so a same-prompt GRPO group becomes a ragged
collection. It therefore replaces group-relative advantages with critic-based PPO: each
rollout trains independently, the critic supplies token-level advantages, and a token-level
loss addresses length imbalance \citep{glm52team2026}. This removes the sibling sampling
needed to estimate a GRPO baseline, not rollout sampling, and densifies credit even when
the external reward remains terminal. Anti-hacking is separate: rules and an LLM judge
screen suspicious tool calls, and a blocked call receives dummy information so its rollout
can continue.

\subsection{Why reconstructed context can invalidate a policy ratio}
